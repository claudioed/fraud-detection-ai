...
public class Setup {
    ...
    public void init(@Observes StartupEvent ev, CustomerRepository customers, TransactionRepository transactions) {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            transaction.customerId = String.valueOf(customers.findAll().page(idx, 1).firstResult().id);
            transaction.amount = BigDecimal.valueOf(random.nextInt(1000) + 1);
            transaction.at = LocalDateTime.now().minusMinutes(random.nextInt(20));
